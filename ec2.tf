...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "git pull",
      "ansible-playbook robo.yml -e HOST=localhost -e ROLE=${var.COMPONENT} -e ENV=${var.ENV} -e DOCDB_ENDPOINT=${var.DOCDB_ENDPOINT} -e REDDIS_ENDPOINT=${var.REDDIS_ENDPOINT}  -e MYSQL_ENDPOINT=${var.MYSQL_ENDPOINT}",
    ]
